...
class Menu:
    ...
    def start(self) -> None:
        ...
        if inp == "1" or inp == "pvp":
            ...
            try:
                PvC(self.difficulties_dict[input(f"choose bot difficultly: ")]).play()
            except KeyError:
                print(f"Invalid difficulty alias. Valid difficulty aliases: '{self.difficulties_dict}'.")

        elif inp == "3" or "cvc":
            try:
                CvC(self.difficulties_dict[input(f"choose first bot difficultly: ")],
                    self.difficulties_dict[input(f"choose second bot difficultly: ")],
                    float(re.match(self.delay_pattern, input(f"Delay (in seconds) between every turn: ")).string)
                    ).play()
            except KeyError:
                print(f"Invalid difficulty alias. Valid difficulty aliases: '{self.difficulties_dict}'.")
            except AttributeError:
                print(f"Invalid delay value. RE pattern for this input: '{self.delay_pattern.pattern}'.")
